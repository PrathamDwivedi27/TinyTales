      ...
      <div className="grid grid-cols-1 md:grid-cols-2 mt-10 gap-10 items-center">
        <div>
          {options.map((option) => (
            <div
              key={option.id}
              className={`p-6 my-3 text-center rounded-lg text-white cursor-pointer hover:scale-105 transition-all ${
                selectedOption === option.id ? 'bg-black' : 'bg-primary'
              }`} // Ensure correct comparison
              onClick={() => setSelectedOption(option.id)} // This correctly sets the selected option
            >
